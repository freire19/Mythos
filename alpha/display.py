...
def print_banner(provider: str, model: str) -> None:
    """Display the Mythos startup banner — Kali Linux inspired."""
    cwd = os.getcwd()
...
def label_for_tool(name: str) -> str:
    ...
    if n in {"query_database", "describe_table"}:
        return "Querying"
    if n in {"scan_vulnerabilities", "audit_dependencies", "analyze_binary", "fuzz_endpoint", "check_misconfigurations"}:
        return "Scanning"
    if n in {"nmap_scan", "ffuf_fuzz", "banner_grab", "payload_inject", "traffic_capture", "port_knock", "exploit_loop"}:
        return "Attacking"
    return "Working"
